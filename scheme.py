...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if not check_procedure(first_eval):
            if isinstance(first_eval, MacroProcedure):
                return scheme_eval(first_eval.apply_macro(rest, env), env, True)
            else:
                return scheme_apply(first_eval, rest.map(lambda x: scheme_eval(x, env)), env)
        # END PROBLEM 5
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("TypeError args: ")
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        sec_val = scheme_eval(expressions.second.first, env)
        env.define(target, sec_val)
        return target 
        # END PROBLEM 6
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        eval_variable = scheme_eval(expressions.first, env)
        if scheme_symbolp(expressions.first) and scheme_falsep(env.lookup(expressions.first)):
            return env.lookup(expressions.first)
        elif scheme_falsep(eval_variable):
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        ...
        check_first = scheme_eval(expressions.first, env)
        if scheme_symbolp(expressions.first) and scheme_truep(env.lookup(expressions.first)):
            return env.lookup(expressions.first)
        elif check_first is 0:
            return 0
        elif scheme_truep(check_first):
            return check_first
        else:
...
def make_let_frame(bindings, env):
    ...
    list_of_formals = nil
    list_of_values = nil
    while bindings.second != nil:
        check_form(bindings.first, 2, 2)
        list_of_formals = Pair(bindings.first.first, list_of_formals)
        list_of_values = Pair(scheme_eval(bindings.first.second.first, env), list_of_values)
        check_formals(list_of_formals)
        bindings = bindings.second
    if bindings.first != nil:
        check_form(bindings.first, 2, 2)
        list_of_formals = Pair(bindings.first.first, list_of_formals)
        list_of_values = Pair(scheme_eval(bindings.first.second.first, env), list_of_values)
        check_formals(list_of_formals)
    return env.make_child_frame(list_of_formals, list_of_values)
    # END PROBLEM 15
